...
def parse_input(input_pattern: str, bsort_queues: bool = True):
  ...
  for pieces_format, permutate_format in pattern_parts:
    ...
    # is a set of pieces
      ...
      tmp_match_obj := re.match(
        r"\[\^?([TILJSZO]*(\[[TILJSZO]+\])*[TILJSZO]*)\]", pieces_format
      )
    ) is not None:
      ...
      if permutate_format == "!":
        queues.append(
          set(
            map(
              "".join,
              chain.from_iterable(map(permutations, actual_pieces)),
            )
          )
        )
          ...
          queues.append(
            set(
              map(
                ...
                chain.from_iterable(
                  map(
                    lambda x: permutations(x, permutate_num),
                    actual_pieces,
                  )
                ),
              )
...
def make_modifier_tree(
  modifier: str, index: int = 0, depth: int = 0, return_length: bool = False
) -> list | tuple[list, int]:
  """
  ...
  for modifierPart in modifierTree:
    ...
    # handle the modifiers
      ...
      if countModifierMatchObj := re.match(
        ...
        countPiecesParts = filter(None, re.split(r"(\[[TILJSZO*]+\])", countPieces))
      ...
      # before modifier
        ...
        # separate any set notation
        beforePiecesParts = filter(None, re.split(r"(\[[TILJSZO*]+\])", beforePieces))
        afterPiecesParts = filter(None, re.split(r"(\[[TILJSZO*]+\])", afterPieces))
