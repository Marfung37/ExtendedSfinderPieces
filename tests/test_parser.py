...
@pytest.mark.parametrize(
  ...
  [
    ...
    ("[L[SZ]]", "Generator(['L', 'SZ']p1)"),
    ("[L[SZ]]!", "Generator(['L', 'SZ']p2)"),
    ("[[TI]L[SZ]]!", "Generator(['L', 'TI', 'SZ']p3)"),
    ("[L[^SZ]]!", "Generator(['L', 'TILJO']p2)"),
    ("[^TI[LJ]]!", "Generator(['L', 'J', 'S', 'Z', 'O', 'LJ']p6)"),
    ("[^TILJSZ[O]]!", "Generator(['O', 'O']p2)"),
    (
      "[^TIJ[^LJO]]!",
      "Generator(['L', 'S', 'Z', 'O', 'TISZ']p5)",
    ),
    ("[^TILJSZ[^O]]!", "Generator(['O', 'TILJSZ']p2)"),
    # filter expressions
    ...
    ("{T<L}", "Filter(Before(['T'] < ['L']))"),
    ("{IT<[LJ]}", "Filter(Before(['I', 'T'] < ['LJ']))"),
    ("{3:T<L}", "Filter(Range(0-3 -> Before(['T'] < ['L'])))"),
    (
      "{[LJ]=1&&!LJ=1}",
